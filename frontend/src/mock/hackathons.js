let hackathons = [
    {
        id: 0,
        image: 'https://images.unsplash.com/photo-1552799446-159ba9523315?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1740&q=80',
        title: 'Save the Planet hackathon',
        description: 'Do you have what it takes to develop the great next solution to climate change?',
        start_date: 1658318400,
        end_date: 1658491200,
        prizeSum: "$50k ETH",
        announcements: [
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
            {
                title: 'Sample announcement',
                description: 'This is a sample announcement. Announcements can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
                date: 1658318400,
            },
        ],
        prizes: [
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
        ],
        moments: [
            {
                title: 'Hackathon start',
                timestamp: 1658318400,
            },
            {
                title: 'Hackathon end, Voting start',
                timestamp: 1658491200,
            },
            {
                title: 'Results announcement', timestamp: 1658491200
            },{
                title: 'Hackathon start',
                timestamp: 1658318400,
            },
            {
                title: 'Hackathon end, Voting start',
                timestamp: 1658491200,
            },
            {
                title: 'Results announcement', timestamp: 1658491200
            },
        ]
    },
    {
        id: 1,
        image: 'https://forkast.news/wp-content/uploads/2021/12/polygon-1260x709.jpg',
        title: 'Polygon hackathon',
        description: 'Come develop on the Polygon chain!',
        start_date: 1658318400,
        end_date: 1658491200,
        prizeSum: "$1M ETH",
        announcements: [
            {
                id: 0,
                title: 'Sample announcement',
                description: 'This is a sample announcement',
                date: 1658318400,
            },
        ],
        prizes: [
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
        ],
        moments: [
            {
                title: 'Hackathon start',
                timestamp: 1658318400,
            },
            {
                title: 'Hackathon end, Voting start',
                timestamp: 1658491200,
            },
            {
                title: 'Results announcement', timestamp: 1658491200
            }
        ]
    },
    {
        id: 2,
        image: 'https://forkast.news/wp-content/uploads/2021/12/polygon-1260x709.jpg',
        title: 'Polygon Summer Hack',
        description: 'Build on Polygon',
        start_date: 1658685324000,
        end_date: 1658771724000,
        prizeSum: "0.001 ETH",
        announcements: [
            {
                id: 0,
                title: 'Start announcement',
                description: 'Hackathon has begun!',
                date: 1658685324000,
            },
            {
                id: 1,
                title: 'Welcome Ceremony',
                description: 'Join us to kick off the hack!',
                date: 1658685324000,
            },
            {
                id: 1,
                title: 'Team Building',
                description: 'Find people to work together :)',
                date: 1658685324000,
            }
        ],
        prizes: [
            {
                name: '1st Place',
                value: '1 ETH',
                content: "The Best Project in the Hackathon"
            },
            {
                name: '2nd Place',
                value: '0.1 ETH',
                content: "Runner-up!"
            },
            {
                name: '3rd Place',
                value: '0.01 ETH',
                content: "Runner-runner-up!"
            },
            {
                name: 'Best Effort',
                value: '0.001 ETH',
                content: "Thanks for joining!"
            }
        ],
        moments: [
            {
                title: 'Hackathon starts',
                timestamp: 1658685324000,
            },
            {
                title: 'Hackathon ends, Voting starts',
                timestamp: 1658771724000,
            },
            {
                title: 'Voting ends, Winners Announced',
                timestamp: 1658771724000,
            },
        ]
    },
    {
        id: 3,
        image: 'https://protocol.ai/images/og-default.jpg',
        title: 'ProtocolAI hackathon',
        description: 'Develop your app and show us the great result!',
        start_date: 1658318400,
        end_date: 1658491200,
        prizeSum: "$1M ETH",
        announcements: [
            {
                id: 0,
                title: 'Sample announcement',
                description: 'This is a sample announcement',
                date: 1658318400,
            },
        ],
        prizes: [
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
            {
                name: 'Sample prize',
                value: '69 USD',
                content: 'This is a sample prize. Prizes can be long and contain multiple lines of text displaying. It is useful to notify participants of updates.',
            },
        ],
        moments: [
            {
                title: 'Hackathon start',
                timestamp: 1658318400,
            },
            {
                title: 'Hackathon end, Voting start',
                timestamp: 1658491200,
            },
            {
                title: 'Results announcement', timestamp: 1658491200
            },
        ]
    },
]

const getSignedUpHackathons = async () => {
    // indexes 0 and 1
    let myhacks = [hackathons[0], hackathons[1]];

    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(myhacks);
        }, 500);
    })
}

const getHackathons = async () => {

    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(hackathons);
        }, 1000);
    })
}

const getHackathon = async (id) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(hackathons[id]);
        }, 300);
    });
}

const getTeam = (id) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve((Math.floor(Math.random() * Math.pow(2, 16))).toString(16));
        }, 300);

    })
}

const getAnnouncements = (hack_id) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(hackathons[hack_id].announcements);
        }, 300);
    });
}

const getPrizes = (hack_id) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(hackathons[hack_id].prizes);
        }, 300);
    });
}

const getMoments = (hack_id) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(hackathons[hack_id].moments);
        }, 300);
    });
}

export { getHackathons, getSignedUpHackathons, getHackathon, getTeam, getAnnouncements, getPrizes, getMoments };
